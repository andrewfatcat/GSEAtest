...
raw=read.csv("leukemia.txt",header=FALSE,sep="\t",row.names = 1,stringsAsFactors = FALSE)
raw=t(raw)
row.names(raw)=NULL
raw=as.data.frame(raw, stringsAsFactors = FALSE)
for(i in 2:ncol(raw)){raw[,i]=as.integer(raw[,i])}
...
for(i in genes){test$p.value[test$gene==i]=t.test(raw.all[,i],raw.aml[,i])$p.value}
for(i in test$gene){test$mean.all[test$gene==i]=mean(raw.all[,i])}
for(i in test$gene){test$mean.aml[test$gene==i]=mean(raw.aml[,i])}
...
test$qvalue[test$sign>0]=log10(test$p.value[test$sign>0])/log10(min(test$p.value[test$sign>0]))*test$sign[test$sign>0]
test$qvalue[test$sign<0]=log10(test$p.value[test$sign<0])/log10(min(test$p.value[test$sign<0]))*test$sign[test$sign<0]
test$ranksign=rank(-test$qvalue)
barplot(test$qvalue[order(test$ranksign)])

#import pathways
pathway=read.csv(file="pathways.txt",sep = "\t", header=FALSE,stringsAsFactors = FALSE)
#get pathway with more than 15 genes
for(i in row.names(pathway)){
    if(length(as.character(pathway[row.names(pathway)==i,3:21])[as.character(pathway[row.names(pathway)==i,3:21])!=""])<15){
...
for(k in report$geneset){
	...
	hitgene=NULL
	for(i in genes){if(i %in% pathwaygene == TRUE){hitgene=c(hitgene,i)}}
	#calculate ES
	NR=0
	p=1
	for(i in hitgene){NR=NR+abs(test$qvalue[test$gene==i])^p}
	ranks=order(test$ranksign)
	phit=0
	pmiss=0
	ESs=NULL
	for(i in ranks){
    	if(test$gene[i] %in% hitgene == TRUE){phit=phit+abs(test$qvalue[i])^p/NR}
    	if(test$gene[i] %in% hitgene == FALSE){pmiss=pmiss+1/(length(genes)-length(hitgene))}
    	ESs[i]=phit-pmiss}
	test$ES=ESs
	...
	for(j in 1:nper){rankper[j,]=sample(ranks)}
	for(j in 1:nper){
    		phit=0
    		pmiss=0
    		m=rankper[j,]
...
if(ES>=0){esmean=mean(esper[esper>=0])}
if(ES<0){esmean=mean(esper[esper<0])}
report$esmean[report$geneset==k]=esmean
#calculate p-value for ES
if(ES>=0){esp=1-ecdf(esper[esper>=0])(ES)}
if(ES<0){esp=ecdf(esper[esper<0])(ES)}
